...
//where any processed code or vendor files gets placed for user in development
...
var cfg={
	//OUr client application source code globs and build paths
	...
	apiUrl: { dev: "http://localhost:3000",
			  prd: "https://murmuring-woodland-38077.herokuapp.com" },
...
//run the browser against the development/build area and watch the files being edited
...
gulp.task("dist:assets", ["build"], function(){
	return gulp.src(cfg.root_html.src).pipe(debug())
		...
		.pipe(gulpif(["**/*.js"], replace(cfg.apiUrl.dev, cfg.apiUrl.prd))) // modified from above
